...
class UserGuideContentTest {
    ...
    @Test fun `update highlights never fall back for known versions`() {
        ...
        for (version in listOf("8.3.0-rc.22", "8.3.0-rc.21", "8.3.0-rc.20", "8.3.0-rc.19", "8.3.0-rc.18", "8.3.0-rc.17", "8.3.0-rc.16", "8.3.0-rc.15", "8.3.0-rc.14", "8.3.0-rc.13", "8.2.2", "8.2.1", "8.2.0", "8.1.1", "8.1.0", "7.12.0", "7.9.0-rc.2", "7.9.0")) {
    ...
    @Test fun `8_3_0_rc_13 highlights explain shared glass opacity and legacy defaults`() {
        val copy = updateHighlightsFor("8.3.0-rc.13").joinToString("\n")
        for (keyword in listOf("亚克力", "毛玻璃", "40%–95%", "60%", "48%", "课表")) {
            assertTrue("rc.13 更新说明应提到「$keyword」", copy.contains(keyword))
        }
    }

    @Test fun `8_2_1 highlights distinguish acrylic and frosted glass`() {
        val copy = updateHighlightsFor("8.2.1-rc.7").joinToString("\n")
        for (keyword in listOf("亚克力", "毛玻璃", "丰富外观", "默认关闭")) {
            assertTrue("8.2.1 更新说明应提到「$keyword」", copy.contains(keyword))
        }
        assertTrue("亚克力更新说明应明确无描边", copy.contains("无描边"))
        assertTrue("毛玻璃更新说明应明确圆角亮边完整", copy.contains("完整圆角亮边"))
    }
