...
internal val userGuideChapters = listOf(
    ...
    HelpSection("三、日程、提醒与后台", listOf(
        ...
        "重启恢复：系统若推迟了开机启动（ColorOS 实测会推迟），打开一次 FocusFlow 就会补登记提醒；允许自启动可免去这一步，具体路径见设置页问号。",
    ...
    HelpSection("七、导航、返回与动效", listOf(
        ...
        "打开弹窗本身不算一步历史：弹窗、当前副页和已经填进去的内容算同一个整体。所以开着弹窗按上一步＝回到上一个页面（例如从「课程 + 编辑弹窗」回到计划主页）；反过来，从这里跳到别处（管理地点、其他页、本页签主页）之后再按上一步，副页、弹窗和里面的内容一起回来。想真正关掉弹窗，按返回键、点弹窗外面或点取消即可。",
        "系统返回键：不在今日页时先回到今日主页；在今日页第一次按会提示“再按一次返回键退出应用”，4 秒内再按才退出。提示里的“不再提示”可永久关闭，设置 → 退出确认 可以改回来。弹窗打开时，返回键先关闭弹窗。",
        "弹窗是页内浮层，从屏幕下方平移进出，卡片带一层主题色柔光阴影、顶面微亮，明显浮在压暗的页面之上：打开时底栏仍可点；按返回键或点弹窗外面能关闭，点弹窗卡片本身不会误关；键盘弹出时弹窗整体上移，按钮始终可见。",
