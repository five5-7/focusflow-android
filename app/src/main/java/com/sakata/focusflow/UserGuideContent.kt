...
internal val userGuideChapters = listOf(
    ...
    HelpSection("八、外观与主题", listOf(
        "内置七套主题：海盐蓝、薄荷绿、暖杏、暮紫，以及新增的石墨、樱粉、青竹。选择会记住；课程色块与提醒警示保持各自语义，不受主题影响。",
        "页面背景有四档（设置 → 外观 → 页面背景）：跟随主题、主题渐变、固定颜色、自选图片。渐变有自己的“配色”（八组「顶色→底色」，不选则按主题派生）与“强度”滑杆（0% 就是纯色）。",
        "“渐变跟随内容”决定渐变有多长：关＝整条渐变正好一屏，颜色变化快；开＝渐变铺满数屏内容、每屏只走一小段，竖向变化更慢更缓。",
        "自选图片背景：从系统文件选择器挑一张图，可调不透明度；图片只存在本机、不会上传，图上始终压一层主题遮罩保证正文读得清。",
        "“从图片抽取主题色”会按图片里最主要的颜色生成一整套自定义主题（含页面底色与文字色，并保证正文对比度）；图片颜色太素时会如实说明，主题保持不变。",
        "卡片材质五档：默认（原来的纯色卡片）、渐变、柔光、亚克力、毛玻璃。亚克力保留更多背景轮廓并带主题染色；毛玻璃扩散更强、颜色更中性。玻璃类只模糊卡片底层，不会模糊卡片文字。",
        "课表与日程表底色可跟随主题、选颜色、换图片，或者选「透明」让底板整块让开、直接露出页面背景——**只改底板**，课程块与日程块的颜色来自各自数据，不受影响。",
        "自定义主题编辑器里会当场给出对比度体检：正文、卡片、主/副/强调色上的文字逐项检查，不达标会标红说明是哪一处。",
        "编辑器顶部是实时预览：用应用里同一套渲染画一个迷你页面（背景、渐变、卡片材质、底栏），改一下就能当场看到，不必先应用再退出设置页。",
        "背景与卡片外观也能在编辑器里直接调（与「设置 → 外观」是同一批控件、同一份设置）；保存预设时可以勾选“同时记住当前外观”，于是预设从一个配色变成一整套外观，点“应用”一键换整套。老预设（只存了配色）保持原样：应用它不会动你当前的背景与卡片。",
        "编辑器里的“恢复默认主题配色与外观”会回到最近一次用过的内置主题，并把背景、卡片材质、课表底色一并复位。",
        "深色模式下以上外观都会自动适配：渐变幅度另配一套（顶上微微提亮、底下压深），自选的浅色配色会被压到深色底上，整页仍是深色，正文依然读得清。"
    ))
)
