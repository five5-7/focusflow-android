...
internal val userGuideChapters = listOf(
    ...
    HelpSection("二、页面与功能速查", listOf(
        ...
        "想录入课程：计划 → 课程；手动新增可选周一至周日，填写“第几节开始＋连续几节”，一节默认 45 分钟。",
        "想设置地点或通勤：设置 → 通勤与地点；想创建长期目标：计划 → 目标与执行；想测试通知：设置 → 日程与活动提醒。"
    )),
    ...
    HelpSection("四、默认设置与可选能力", listOf(
        "核心记录、收集箱和手动安排不需要 AI、地点、课表或额外权限。新安装没有预置地点；通勤预留默认开启，先按单程 10 分钟估计且不读取定位。",
    ...
    HelpSection("五、课程、地点、通勤与目标", listOf(
        "地点为空时可先继续录入课程；返校后再添加地点或导入地点包。通勤时长和方式可在设置调整，已有设置不会被新版本悄悄重置。",
