...
object RoadmapData {
    ...
    val evolution: List<RoadmapVersion> = listOf(
        RoadmapVersion("8.2.0", listOf(
            RoadmapEntry("8.2.0", "可选外观系统", "页面背景四档（跟随主题／主题渐变／固定颜色／自选图片，图片可调不透明度且只存本机）；渐变可自选「顶色→底色」配色与强度，并可「跟随内容」把跨度拉长到数屏、让竖向变化更缓；卡片材质（默认／渐变／柔光，抽成统一卡片组件并收编 32 处调用点）；课表与日程表底色可单独设置（只改底板，课程块与日程块颜色不动）；新增石墨／樱粉／青竹三套主题（原有四套取值冻结并有金值回归测试）；从图片自动抽取主题色生成一整套自定义主题；自定义主题工具升级：顶部实时预览（走应用同一套渲染）、背景与卡片外观直接在里面调（与「设置 → 外观」同一批控件）、对比度体检当场标红、预设可连整套外观一起存（老预设只存配色、行为不变）、恢复默认连外观一并复位；以上全部适配深色模式（渐变另配一套幅度、自选浅色压到深色底）。默认外观与 8.1.1 逐像素一致（同机 A/B 实测 meanAbsDiff 0.37）。", RoadmapStatus.DONE)
        )),
