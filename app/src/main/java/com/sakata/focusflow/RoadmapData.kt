...
object RoadmapData {
    ...
    val evolution: List<RoadmapVersion> = listOf(
        RoadmapVersion("7.8.0", listOf(
            RoadmapEntry("7.8.0-rc.3", "课表信息密度与视图记忆", "缩小视图在可用宽度内显示完整一周；周五至周日均无课时合并为空白区，只要其中一天有课便恢复逐日显示。标准视图充分利用色块显示课程名称与地点；应用自动保存上次选择，首次默认使用缩小视图。", RoadmapStatus.CANDIDATE),
