...
object RoadmapData {
    ...
    val future: List<RoadmapEntry> = listOf(
        RoadmapEntry("7.6.0", "课程新增、地点与出行参数易懂化", "补充课程新增就近帮助；星期明确支持周一至周日；输入改为“第几节开始＋连续几节”，每节默认 45 分钟且可调整；“教学楼”统一改称“地点”，提供设置→通勤与地点入口；新安装默认地点包为空、课程出行参数默认开启、默认预留 10 分钟；调整滑动条范围、刻度与可读性，并验证旧数据迁移及跨界面文案一致性", RoadmapStatus.PLANNED),
