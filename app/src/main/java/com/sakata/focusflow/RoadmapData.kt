...
object RoadmapData {
    ...
    val evolution: List<RoadmapVersion> = listOf(
        RoadmapVersion("8.1.0", listOf(
            RoadmapEntry("8.1.0-rc.6", "统一动效规范与深度缩放转场", "把散落的时长与缓动收拢为 MotionSpec（进入 240ms／退出 170ms／位移 260ms／底栏形变 260ms），全部经「设置 → 外观 → 动画速度」换算，关闭时瞬时完成；副页改为从底栏图标深度缩放进出，主页之间左右平移；底栏选中底色改为会平移的色块、进入子页时显示圆环；弹窗改为页内浮层（打开时底栏亮起柔光仍可点，卡片上下平移进出，点入口／上一步或返回键先关弹窗，键盘弹出自动上移）；深链／通知直接打开子页照常从图标放大；转场判定抽为 TabMotionRules 并补单测；导航历史改为任何页面切换都记一步（上一步能回到刚才的页面）；修子页转场每帧重组导致的掉帧、离开子页后点击被吞、横屏弹窗按钮被底栏盖住、退出提示时长与冷启动返回兜底；应用锁定竖屏。", RoadmapStatus.CANDIDATE),
