...
object RoadmapData {
    ...
    val evolution: List<RoadmapVersion> = listOf(
        RoadmapVersion("8.3.0", listOf(
            RoadmapEntry("8.3.0-rc.9", "权限、浙大课表自动导入与首次动画性能", "权限与提醒集中检查且如实标出 ColorOS 手动项；浙大课表在应用内输入凭据后自动登录、读取并同步，认证和教务会话分段处理；导入页改为跟随主题的 Compose 卡片界面，提供密码显隐、隐私提示、七段进度与对应失败状态；凭据只驻留内存，新课进入候选，唯一匹配的已有课程更新，歧义项不覆盖；启动数据移出首次组合并延后隐藏页预热。浙大导入 UI 与帧耗时仍待真机复测。", RoadmapStatus.CANDIDATE)
        )),
