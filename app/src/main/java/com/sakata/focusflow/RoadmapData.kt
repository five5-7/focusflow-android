...
object RoadmapData {
    ...
    val future: List<RoadmapEntry> = listOf(
        RoadmapEntry("后续", "课程手动新增与出行参数易懂化", "补充课程新增帮助；星期可明确选择周一至周日（包含周五、周六、周日）；将难懂的开始／结束节次改为“第几节开始＋连续几节”，一节默认 45 分钟并允许按实际课程调整；“教学楼”改称“地点”，提供前往设置→通勤与地点的入口；默认地点包为空，课程处默认启用出行参数、默认预留 10 分钟，并调整相关滑动选择条的范围、刻度与可读性。具体默认值实施前需核对旧数据兼容与全局文案一致性", RoadmapStatus.CANDIDATE),
        RoadmapEntry("后续", "默认设置可解释与零经验快速入门", "审计所有带默认值、默认开启／关闭或自动行为的界面，在就近帮助、设置说明、快速入门和版本路线中统一解释“默认是什么、为什么、如何修改、影响什么”；快速入门按首次使用主流程重写，使零经验用户能理解产品用途、收集箱→安排／推进→日程→完成／恢复的使用方法，以及课程、地点、通勤、提醒和隐私边界", RoadmapStatus.CANDIDATE),
        RoadmapEntry("后续", "应用内更新完成提示", "每次版本更新后仅提示一次当前版本号与核心变化，并提供跳转到版本路线的入口；覆盖安装不重复打扰，提示内容必须读取真实版本与更新记录，不写死旧版本文案", RoadmapStatus.CANDIDATE),
