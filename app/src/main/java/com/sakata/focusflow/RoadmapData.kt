...
object RoadmapData {
    ...
    val evolution: List<RoadmapVersion> = listOf(
        RoadmapVersion("8.3.0", listOf(
            RoadmapEntry("8.3.0-rc.8", "权限、浙大课表自动导入与首次动画性能", "权限与提醒集中检查且如实标出 ColorOS 手动项；浙大课表在应用内输入凭据后自动登录、读取并同步。认证提交与教务会话建立分段显示，到达教务域名后不等待无用页面正文，修复验证账号超时并提供分阶段错误；凭据只驻留内存，新课进入候选，唯一匹配的已有课程更新，歧义项不覆盖；启动数据移出首次组合并延后隐藏页预热。浙大实际账号与帧耗时仍待真机复测。", RoadmapStatus.CANDIDATE)
        )),
