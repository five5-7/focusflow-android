...
object RoadmapData {
    ...
    val evolution: List<RoadmapVersion> = listOf(
        RoadmapVersion("8.2.1", listOf(
            RoadmapEntry("8.2.1-rc.1", "亚克力与毛玻璃扩展", "卡片材质扩为默认／渐变／柔光／亚克力／毛玻璃五档。亚克力以 18dp 模糊保留背景轮廓并叠加主题染色；毛玻璃以 30dp 模糊做更强扩散，配低染色半透明白纱和窄高光内边。悬浮底栏与页内弹窗按真实位置采样、透出并模糊下方日程内容，前景文字和按钮保持清晰；旧存档按原键兼容。丰富外观总开关继续默认关闭，主动开启后才参与渲染。", RoadmapStatus.CANDIDATE)
        )),
