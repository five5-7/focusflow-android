...
object RoadmapData {
    ...
    val evolution: List<RoadmapVersion> = listOf(
        RoadmapVersion("8.0.0", listOf(
            RoadmapEntry("8.0.0", "第三个正式版", "集中整理今日状态、收集箱、课表与课程、通勤、空挡、目标和历史，并修复计划页闪退及任务状态可能被旧页面覆盖的问题；当前正在完成发布前最后验证。", RoadmapStatus.CANDIDATE),
            RoadmapEntry("8.0.0-rc.4", "过去的安排不再占用未来", "空挡按今天起的七天计算：昨天的任务不会继续挡住下周同一天，今天已经过去的时间也不会再次推荐；跨过午夜仍在进行的安排照常保留。", RoadmapStatus.CANDIDATE)
        )),
