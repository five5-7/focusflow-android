...
object RoadmapData {
    ...
    val evolution: List<RoadmapVersion> = listOf(
        RoadmapVersion("8.3.0", listOf(
            RoadmapEntry("8.3.0-rc.7", "权限、浙大课表自动导入与首次动画性能", "权限与提醒集中检查且如实标出 ColorOS 手动项；从“从教务网导入 → 浙江大学”进入，在应用内输入凭据后由原生 HTTPS 流程自动登录、读取当前学期与课表并显示进度，凭据只驻留内存；新课进入候选，唯一匹配的已有课程更新，歧义项不覆盖；启动数据移出首次组合并延后隐藏页预热。浙大账号、异常登录路径与帧耗时真机验收仍待完成。", RoadmapStatus.CANDIDATE)
        )),
