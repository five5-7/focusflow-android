...
object RoadmapData {
    ...
    val future: List<RoadmapEntry> = listOf(
        RoadmapEntry("后续", "课程手动新增与出行参数易懂化", "补充课程新增帮助；星期可明确选择周一至周日（包含周五、周六、周日）；将难懂的开始／结束节次改为节数表达，一节默认 45 分钟；“教学楼”改称“地点”，提供前往设置→通勤与地点的入口；默认地点包为空，课程处默认启用出行参数、默认预留 10 分钟，并调整相关滑动选择条的范围、刻度与可读性。具体默认值实施前需核对旧数据兼容与全局文案一致性", RoadmapStatus.CANDIDATE),
