...
object RoadmapData {
    ...
    val evolution: List<RoadmapVersion> = listOf(
        RoadmapVersion("7.6.0", listOf(
            RoadmapEntry("7.6.0-rc.1", "课程新增、地点与出行参数易懂化", "手动新增课程支持周一至周日，并改为“第几节开始＋连续几节”；一节默认按 45 分钟计算。“教学楼”统一为“地点”，可直接跳转设置管理地点与出行参数。新安装不预置地点包，默认开启通勤预留且单程 10 分钟（旧设置保留）。通勤时长使用 5／10／15／20／30／45／60 分钟常用档位；自定义地点只填写名称与用途，新增“其他”，分区自动推断且不再显示经纬度字段。已通过 CI 构建，仍待真机验收。", RoadmapStatus.CANDIDATE)
        )),
    ...
    val future: List<RoadmapEntry> = listOf(
        RoadmapEntry("7.7.0", "默认设置可解释与零经验快速入门", "全应用审计默认值、默认开关与自动行为，在就近帮助、设置、快速入门和版本路线统一解释“默认是什么、为什么、如何修改、影响什么”；按首次使用主流程重写快速入门，使零经验用户理解产品用途、收集箱→安排／推进→日程→完成／恢复，以及课程、地点、通勤、提醒、数据与隐私边界", RoadmapStatus.PLANNED),
        RoadmapEntry("7.6.0", "应用内更新完成提示", "覆盖安装或首次进入新版本后仅提示一次真实版本号与核心变化，并提供跳转到版本路线的入口；同一版本不重复打扰，提示内容读取真实版本与更新记录，不写死旧版本文案", RoadmapStatus.PLANNED),
