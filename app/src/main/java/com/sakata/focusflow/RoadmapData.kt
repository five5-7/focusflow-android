...
object RoadmapData {
    ...
    val evolution: List<RoadmapVersion> = listOf(
        RoadmapVersion("8.3.0", listOf(
            RoadmapEntry("8.3.0-rc.6", "权限、浙大课表导入与首次动画性能", "权限与提醒集中检查且如实标出 ColorOS 手动项；通过浙大官方统一身份认证页读取当前课表，不保存账号密码，导入后仍逐项确认；启动数据移出首次组合并延后隐藏页预热。浙大账号与帧耗时真机验收仍待完成。", RoadmapStatus.CANDIDATE)
        )),
