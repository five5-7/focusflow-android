...
object HelpCatalog {
    ...
    val settings: Map<SettingsBlock, HelpSection> = mapOf(
        SettingsBlock.APPEARANCE to HelpSection("外观", listOf(
            ...
            "卡片材质：默认／渐变／柔光／亚克力／毛玻璃。亚克力轻度模糊并带主题色，不加描边；毛玻璃模糊更强，并有完整的圆角亮边。两者都不会模糊文字。课表与日程表底色可单独设置，只改底板，不改变课程块和日程块。",
        ...
        SettingsBlock.COURSE_VISION to HelpSection("课表识别（视觉模型）", listOf(
            ...
            "截图必须同时保留顶部星期标题和左侧连续节次；模型返回的表头文字、网格坐标与课程色块位置会相互校验。缺表头、横行误作星期、网格转置或位置无法对应时会停止整批导入，请重新截图而不要反复尝试同一张裁切图。",
