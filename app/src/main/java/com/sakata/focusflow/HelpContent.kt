...
object HelpCatalog {
    ...
    val plan = PageHelp("计划", listOf(
        ...
        HelpSection("课程", listOf("可从课表截图识别，也可手动新增。手动新增选择周一至周日，填写“第几节开始＋连续几节”；一节默认 45 分钟。地点用于课程显示和已开启的出行时间估算，没有地点包时可直接自填。", "截图识别需先在 设置→高级工具→课表识别（视觉模型） 开启并填写 key；结果先进入待确认区，不会直接加入日程。截图中需要显示课程名称、星期和节次。确认课程后，它们会用于周日程和空挡计算。")),
