...
object HelpCatalog {
    ...
    val settings: Map<SettingsBlock, HelpSection> = mapOf(
        ...
        SettingsBlock.STATUS_CHECK_IN to HelpSection("状态询问", listOf(
            ...
            "签到数据仅保存在本机；不会自动修改询问时刻或日程，调整必须由你主动确认。睡眠等外部背景只有在单独授权可靠数据源后才考虑，不要求每次手填。"
        )),
        SettingsBlock.SLEEP_DATA to HelpSection("睡眠数据", listOf(
            "睡眠数据接口默认关闭，只通过 Health Connect 请求读取睡眠，不申请写入或其他健康数据权限。",
            "同步时检查最近 7 天的睡眠记录与实际来源应用，再从最近 36 小时选择至少 3 小时的主要睡眠；只在本机保存摘要。",
            "授权成功不代表手环已经接通。小米运动健康、华为运动健康等厂商应用必须先把睡眠写入 Health Connect；页面会区分无权限、无来源数据、有旧记录和同步成功。",
            "没有授权、没有同步记录或读取失败时，精力记录与日程功能保持原样；FocusFlow 不会要求你每次手填睡眠。",
            "睡眠只能作为精力建议的背景，不能覆盖你主动记录的感受，也不能自动移动固定日程。"
        )),
