...
internal fun updateHighlightsFor(version: String): List<String> = when {
    version.startsWith("8.1.0") -> listOf(
        "页面转场统一重做：子页从底栏图标放大展开、返回时缩回该图标；主页之间左右平移。设置 → 外观 新增「动画速度」，可关闭或调快调慢。",
        "底栏选中底色改为会滑动的色块，进入子页时显示圆环；回退／折返仍是两端圆瓣形变，长按回退键可弹会话历史列表。",
        "根页面按返回先提示“再按一次返回键退出应用”，4 秒内再次返回才退出；弹窗不再挡住底栏——打开时仍能点今日／日程／计划／设置和回退／折返键，按返回键或点弹窗外面关闭，键盘弹出时自动上移；填了一半的弹窗被误关后重开仍会恢复内容。"
    )
