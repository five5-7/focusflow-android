...
internal val quickStartChapters = listOf(
    HelpSection("日常流程", listOf(
        "点底部＋，用“快速记录”收集一件想做的事。",
        "在今日页收集箱直接安排时间；暂时不适合排时间的内容，可整理为“逐步推进”或“参考”。",
        "逐步推进只确认一个下一步；建立后它会作为关联任务进入收集箱，父想法和原说明继续保留，也可在方向卡片直接完成这一步。",
    ...
    HelpSection("提醒权限", listOf(
        "允许通知并优先检查日程渠道；只有开启饭点提醒后才需要检查饭点渠道。不同系统可能称为横幅、悬浮或弹出窗口。",
        "应用会读取系统精确闹钟能力；需要授权时按系统提示设置，无法使用时会明确降级。",
        "在设置的日程与活动提醒里运行 1 分钟测试。回到桌面等待，不能把打开应用后的补发当作后台正常。",
        "厂商自启动开关无法可靠自动检测，请按设备说明允许后台活动和自启动。",
        "每日精力询问默认关闭；开启后可在设置查看下一次时间和最近结果，并用一分钟测试验证后台通知。签到历史只给出时刻建议，不会自动改设置。",
        "一次性静音和免打扰只控制已开启的精力询问、饭点和睡前减速；任务与活动到点提醒不被静音。"
    )),
    HelpSection("可选工具", listOf(
        "需要生活建议时再填写习惯基线；上学时可导入或手动新增课表；长期任务再建目标。手动新增课程可选周一至周日，填写“第几节开始＋连续几节”，一节默认 45 分钟；地点和出行参数均可稍后在设置调整。",
        "饭点提醒默认关闭；开启后提醒你好好吃饭。用餐结束询问是另一个默认关闭的开关，只有开启后才追问吃完时间和学习用餐时长。",
        "＋里的安排空闲活动可给游戏、视频、学习或运动设定时间。前台应用检测默认关闭，只增强游戏／视频收尾文案；无法识别时仍正常提醒，绝不会自动结束活动。判断在本机完成。",
