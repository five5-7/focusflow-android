...
internal val quickStartChapters = listOf(
    ...
    HelpSection("页面与功能速查", listOf(
        ...
        "外观都在 设置 → 外观：七套主题、页面背景（跟随主题／渐变／固定颜色／自选图片）、卡片材质、课表与日程表底色。全部可选，不选就是默认样子。",
...
internal fun updateHighlightsFor(version: String): List<String> = when {
    version.startsWith("8.2.0") -> listOf(
        ...
        "渐变可自选配色与强度，也能选「跟随内容」——渐变铺满数屏，滚动时颜色变化更慢更缓。卡片材质增加渐变／柔光／纸感；课表与日程表底色可单独换（只动底板）。",
        "以上外观都适配了深色模式；自定义主题编辑器会当场做对比度体检，读不清的地方会直接指出来。"
    )
