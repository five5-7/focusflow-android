...
开发中：**8.3.0-rc.7**（versionCode 541）——权限与提醒集中检查并如实区分系统可检测项和 ColorOS 手动确认项；浙江大学课表改为应用内输入账号密码后由原生 HTTPS 流程自动登录、读取当前学年／学期并显示分阶段进度，凭据不保存；新课进入候选，唯一匹配的已有课程更新节次与地点；启动数据继续使用后台快照与延后预热。
