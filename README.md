...
开发中：**8.3.0-rc.6**（versionCode 540）——权限与提醒集中检查并如实区分系统可检测项和 ColorOS 手动确认项；课程可通过浙大官方统一身份认证读取当前课表，FocusFlow 不保存账号密码，导入结果仍统一校验并逐项确认；启动数据改由后台快照一次读取并延后隐藏页预热，减少前几次动画争抢。
...
**课表与空挡**
- 浙江大学教务课表导入：在官方统一身份认证页登录后读取当前选择的学年／学期，不保存账号密码；与截图识别（可选 Qwen3-VL）和手动录入共用待确认、去重与冲突提示
