...
当前发布版本：**8.1.1**（versionCode 521），修「上一步／下一步」与弹窗的层级关系、弹窗柔光与页面分层。上一正式版为 **8.1.0**（versionCode 520）。

> **8.2.0（versionCode 522）已就绪、待发布**：可选外观系统 —— 页面背景四档（跟随主题／主题渐变／固定颜色／自选图片）、渐变配色与强度可调并支持「跟随内容」、卡片材质四档、课表与日程表底色可单独设置、新增石墨／樱粉／青竹三套内置主题、自定义主题工具升级（实时预览／共用外观控件／预设可带整套外观）。**默认外观与 8.1.1 逐像素一致**（同机 A/B 实测 `meanAbsDiff 0.37`），95 个测试类 / 575 条单元测试全绿。详见 `docs/8.2.0-release-notes.md` 与 `docs/8.2.0-checklist.md`。
