...
- 弹窗改为页内浮层：打开弹窗时悬浮底栏（含回退／折返键）仍在最上层、可点，不再被系统窗口整层盖住；返回键与点弹窗外面都能关闭，点弹窗卡片本身不会穿透关闭；键盘弹出时弹窗整体上移到键盘与底栏之上。全部 53 处弹窗统一外观（28dp 圆角、24dp 内边距、同 Material3 字体与按钮排布）。
- 本地崩溃记录（设置 → 稳定性与崩溃：查看／复制／清空，不上传）
- 纯 Kotlin 单元测试覆盖数据兼容、时间轴、规划、提醒策略、条件显示、目标执行依据、导航历史与转场规则（84 个测试类 / 479 条用例）
