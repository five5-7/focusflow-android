...
开发中：**8.3.0-rc.8**（versionCode 542）——权限与提醒集中检查并如实区分系统可检测项和 ColorOS 手动确认项；浙江大学课表使用应用内账号密码与原生 HTTPS 自动导入，认证提交和教务会话分段显示进度，并修复浏览器可访问时仍卡在“验证账号”直至超时的问题；凭据不保存，新课进入候选，唯一匹配的已有课程更新；启动数据继续使用后台快照与延后预热。
