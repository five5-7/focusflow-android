...
- 根页面返回需二次确认（提示 4 秒内再次返回才退出），可点「不再提示」永久关闭，设置 → 退出确认 可改回；非今日页先回今日主页。回退／折返键为底栏两端圆瓣形变，有历史时伸出、无历史时收回：点底栏入口切换主页也记为一步，「上一步」能回到刚才那个页面并恢复当时的子页状态；长按回退键弹会话历史列表；弹窗草稿关闭后重开自动恢复。
...
- 弹窗改为页内浮层：打开弹窗时悬浮底栏（含回退／折返键）仍在最上层、可点，不再被系统窗口整层盖住；底栏会亮起一圈柔光表示"还亮着"，不再有"页面暗、底栏亮"的突兀感；弹窗从屏幕下方上下平移进出；点入口、按上一步／下一步或跳转都会先关闭弹窗；返回键与点弹窗外面都能关闭，点弹窗卡片本身不会穿透关闭；键盘弹出时弹窗整体上移到键盘与底栏之上。全部 53 处弹窗统一外观（28dp 圆角、24dp 内边距、同 Material3 字体与按钮排布）。
