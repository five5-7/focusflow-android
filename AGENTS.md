...
- The current development candidate is `8.3.0-rc.13` / versionCode 547: all rc.12 scope remains; timetable screenshot recognition now requires visible weekday-header and period-label evidence with normalized grid coordinates, and rejects evidence-free arrays, transposed grids, row-as-weekday geometry, or courses that do not snap to the claimed grid. It requires stable-signed CI and OPPO verification and is not merged or released; rc.12 is superseded.
