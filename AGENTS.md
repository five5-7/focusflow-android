...
- The current development candidate is `8.3.0-rc.7` / versionCode 541: centralized permission/reminder guidance; native Zhejiang University import with in-app credential entry, determinate progress, memory-only credentials/session, new-course candidates, and reliable-match updates; startup work remains outside the first composition. It does not bypass CAPTCHA or account unlock flows. By maintainer instruction its RC number continues after rc.4 instead of resetting to rc.1. Target-device login and performance verification remain incomplete. It is not merged or released; rc.6 is superseded.
