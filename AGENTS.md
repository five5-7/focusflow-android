...
- The current development candidate is `8.3.0-rc.11` / versionCode 545: all rc.10 permission, Zhejiang University import, and startup work remains; inbox cards now keep a stable full width, the add menu re-registers after navigation hides it, the course editor uses one scroll owner with adaptive place wrapping, dialog actions wrap on narrow screens, and history arrows move 2dp farther outward. It requires OPPO verification and is not merged or released; rc.10 is superseded.
