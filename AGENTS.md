...
- The current development candidate is `8.3.0-rc.6` / versionCode 540: centralized permission/reminder guidance, Zhejiang University timetable import through the official unified-authentication page, and startup work moved away from the first composition. FocusFlow does not store ZJU credentials; imported courses remain pending confirmation. By maintainer instruction its RC number continues after rc.4 instead of resetting to rc.1. Target-device login and performance verification remain incomplete. It is not merged or released.
