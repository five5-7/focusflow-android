...
- The current development candidate is `8.3.0-rc.10` / versionCode 544: all rc.9 permission-center and Zhejiang University import work remains; for image backgrounds the bitmap is now decoded on IO before the first Compose tree and reused instead of triggering a second full-screen redraw after startup; hidden tabs warm one at a time only after an interaction-aware idle window; reminder restoration and active-session polling no longer read preferences on the main thread. Image + acrylic performance still requires OPPO release verification. It is not merged or released; rc.9 is superseded.
