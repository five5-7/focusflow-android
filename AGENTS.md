...
- The current development candidate is `8.3.0-rc.9` / versionCode 543: centralized permission/reminder guidance; native Zhejiang University import with in-app credential entry, memory-only credentials/session, new-course candidates, and reliable-match updates; authentication submission and ZDBK session establishment are separated with stage-specific timeout reporting and no automatic password resubmission. The ZJU import Activity now uses a theme-aware Compose card UI with password visibility, an explicit privacy notice, seven visual progress steps, and inline failure state. Startup work remains outside the first composition. It does not bypass CAPTCHA or account unlock flows. By maintainer instruction its RC number continues after rc.4 instead of resetting to rc.1. Target-device UI and performance verification remain incomplete. It is not merged or released; rc.8 is superseded.
